...
```{r message=TRUE, warning=TRUE}
# Your code here: 
sw_split_name <- starwars %>% # new data frame
  separate(col = name, # separate 'name' column
           into = c("name_1", "name_2"), # new column names 
           sep = "-") # separate at first dash
# Note: row 49 had 2 dashes. Only 2 pieces expected, third name after second dash was dropped.
```
...
```{r message=TRUE, warning=TRUE}
# Your code here:
sw_skin_sep <- starwars %>% # make a new data frame
  separate(col = skin_color, # separate column 'skin_color'
           into = c("skin_1","skin_2"), # new column names
           sep = ",", # separate at first comma
           extra = "merge", # merge everything after first comma in second column
...
```{r}
# Your code here: 
mpg_model_class <- mpg %>%  # make and name new data frame
  unite(col = model_class, # new column
         c(model, class), #combine model and class
...
```{r}
# Your code here: 
some_dates <- c(04131992,05192003,09302016) # create a vector of dates
some_dates_converted <- mdy(some_dates) # convert to Date with lubridate::mdy
some_dates_converted # call the new vector
...
```{r}
# Your code here: 
gasol_hookshots <- lakers %>%  # new data frame
  filter(player == "Pau Gasol" & type == "hook") %>% #hook shots by Pau Gasol
  mutate(new_date = ymd(date)) %>% # new column with Date class dates
  mutate(month = month(new_date)) %>%  #new column for months 
  count(month) # count shots per month
gasol_hookshots #View it
```
...
```{r}
# Your code here: 
storms_danger <- storms %>% # new data frame
  mutate( danger = case_when( # new column called danger
    status == "hurricane" ~ "high", # if status is hurricane, danger is high
    TRUE ~ "moderate", # all else (status not hurricane), dnager is moderate
  ))
